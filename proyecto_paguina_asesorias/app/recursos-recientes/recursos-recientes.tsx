...
export default function RecentResources() {
  ...
  const filteredResources = allResources.filter((resource) => {
    const matchType =
      selectedType === "all" || resource.tipo_archivo === selectedType;
    const matchSemester =
      selectedSemester === "all" ||
      String(resource.materia?.semestre) === selectedSemester;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
          ...
          <div className="flex flex-row flex-wrap items-center gap-4">
            <div className="flex flex-row items-center gap-2">
              <span className="text-sm font-semibold text-blue-900 dark:text-zinc-300 whitespace-nowrap">
                Tipo de recurso:
              </span>
              <Select
                value={selectedType}
              ...
              return (
                ...
                >
                  <div className={`h-32 ${config.bg} relative overflow-hidden`}>
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="bg-white/90 text-gray-800 px-3 py-1 rounded-full text-xs font-bold">
                        {resource.tipo_archivo === "LINK"
                          ? "ENLACE"
                          : resource.tipo_archivo === "CODE"
                            ? "CÓDIGO"
                  ...
                  <CardContent className="p-5">
                    <h3 className="font-bold text-[#003366] dark:text-zinc-100 mb-2 line-clamp-2 min-h-[48px] text-base">
                      {resource.titulo}
                    </h3>
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500 dark:text-zinc-400">
                      ...
                      <div className="flex items-center gap-1">
                        <RiDownload2Line />
                        <span>{resource.descargas_count}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
